...
const getMovieDetailById = async (id: number) => {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${id}?append_to_response=keywords%2Ccredits%2Csimilar%2Crecommendations%2Creviews&language=en-US`,
    options
  );
...
export default getMovieDetailById;
