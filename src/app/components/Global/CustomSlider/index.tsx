    ...
    <>
      <div className="relative mb-32 xxs:mb-36 sm:mb-48 lg:mb-56">
        {/* {data.map((item,index:number) => {
          return (
            <div key={item.id} className={`opacity-0 text-white transform -translate-x-4 ${index==current-1?style.info:'absolute'}`}>
              <h5>
                {data[current-1]?.name
                  ? data[current-1]?.name
        ...
        <div
          className={`${style.slider} w-full h-full flex justify-center items-end absolute left-1/2 -bottom-[45%] xs:-bottom-[25%] sm:-bottom-[30%] md:-bottom-[35%] xm:-bottom-[25%] transform -translate-x-1/2 z-10`}
        >
